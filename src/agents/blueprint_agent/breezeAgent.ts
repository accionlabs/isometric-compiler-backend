...
const generateBreezeSpec = async (scenarios: string[], context: string): Promise<any> => {
    ...
    return llmServiceInstance.generateJsonWithConversation(__BLUEPRINT_PROMPT__, placeholders, LLM_PLATFORM.OPENAI_MATURE);
};
...
const linkQumWithBreezeSpec = async (scenarios: string[], blueprint: Blueprint): Promise<any> => {
    ...
    return llmServiceInstance.generateJsonWithConversation(__MAPPER_PROMPT__, placeholders, LLM_PLATFORM.OPENAI);
};
